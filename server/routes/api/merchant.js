...
router.post('/seller-request', async(req, res) => {
    try {
        ...
        await mailtrap.sendEmail(email, 'merchant-application');
...
const createMerchantUser = async(email, name, merchant, host) => {
    ...
    if (existingUser) {
        ...
        await mailtrap.sendEmail(email, 'merchant-welcome', null, name);
        ...
        await mailtrap.sendEmail(email, 'merchant-signup', host, {
            resetToken,
            email
        });
